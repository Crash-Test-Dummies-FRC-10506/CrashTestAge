 ...
 * {@link TaskScheduler} owns an instance, and uses it each tick to check if a
 * task it wants to run owns all of its locks.
 */
public final class LockHandler {
     ...
     * task owns all of its keys, it is allowed to execute. If another task has
     * prior claim over a key but has lower priority, ownership of that key is
     * transferred to the other task. A task has to own all keys to take
     * ownership of them.
     *
    ...
    public boolean ownsLocks(BoundTask task) {
        ...
        if (task.lock() == null || !task.handle().state.isExecuting()) {
            return true;
        }
