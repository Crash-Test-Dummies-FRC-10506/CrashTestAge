...
/**
 * The class responsible for orchestrating tasks. The scheduler passes scheduled
 * tasks down to loops through its {@link LoopStore} and has no logic itself for
 * scheduling or handling tasks.
 */
public final class TaskScheduler {
     ...
     * @param lockPriority The task's lock priority. Must be a value from the
     * {@link LockPriority} interface.
     * @param loop The {@link Loop} to write to.
     *
     * @throws NullPointerException If any of {@code job}, {@code persistence},
     * {@code observer}, or {@code loop} are null.
    ...
    /**
     * Schedule a default command. Default commands have the lowest possible
     * lock priority and thus will be suspended if another command shares a lock
     * key. This method always uses the
     * {@link Observer#DEFAULT default observer}, meaning if the command
     * finishes it will be rescheduled.
     *
     * @param command The command to schedule.
     * @param persistence The task's persistence.
     *
     * @throws NullPointerException If {@code command} or {@code persistence}
     * are null.
     */
    ...
    /**
     * Schedule an auto command. Similarly to default commands, auto commands
     * are not bound to any buttons. They however have the highest task
     * priority, meaning other tasks are not able to take ownership of their
     * locks. They also use the {@link Observer#NO_OP
     * no-op observer}, allowing them to terminate once the command finishes.
     * Auto commands are also always
     * {@link TaskPersistence#EPHEMERAL ephemeral}.
     *
     * @param command The command to schedule.
     *
